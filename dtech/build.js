...
function renderFocus(project) {
  ...
  const unresolved = Array.isArray(project.unsolved) && project.unsolved[0] ? project.unsolved[0] : "No open decision recorded.";
  return `<article class="resume-card">
    <div class="resume-main"><p class="focus-label"><span class="signal-dot"></span> Current focus</p><p class="eyebrow">Priority ${esc(number(project.priority))} · ${esc(statusLabel(project.status))} · last touched ${esc(formatDate(project.updated || project.created))}</p><h2>${text(project.title, "Untitled project")}</h2><p class="resume-note">${esc(project.notes || project.targetUser || "Current portfolio focus.")}</p></div>
    <section class="next-move"><span class="label">Do this next</span><p>${esc(nextAction)}</p><a class="text-link" href="#projects">Open the full project brief <span aria-hidden="true">↓</span></a></section>
    <div class="resume-checks"><section><h3>Who it helps</h3><p>${text(project.targetUser)}</p></section><section><h3>Already learned</h3>${shortList(project.solved)}</section><section><h3>Still to decide</h3><p>${esc(unresolved)}</p></section></div>
  </article>`;
}
...
<body>
  ...
  <header class="hero"><div class="shell">
    <nav class="topbar" aria-label="Dashboard navigation"><a class="brand" href="#top"><span class="brand-mark">D</span><span><b>Build OS</b><small>Dementia tech lab</small></span></a><div class="top-links"><a href="#now">Resume</a><a href="#projects">Projects</a><a href="#research">Evidence</a></div></nav>
  <div class="hero-grid" id="top"><div><p class="eyebrow amber">A practical dementia-tech workspace</p><h1>Make care<br><em>simpler.</em></h1><p class="hero-copy">Turn recurring caregiver frustrations into small, testable products—one focused project at a time.</p></div><aside class="hero-note"><p class="eyebrow">When you come back</p><b>This is your resume point.</b><p>It records what this lab is for, what has been learned, and the one move to make next.</p><hr><p class="muted">Last research scan: ${esc(formatDate(latest && latest.date))} · ${esc(projects.length)} saved projects</p></aside></div>
  </div></header>
  <main class="shell">
    <section id="now" class="now-section" aria-labelledby="now-title">${sectionHeader("Resume", "Pick up here", "You do not need to review the whole dashboard first.", "now-title")}${renderFocus(focusProject)}<div class="quick-stats"><span><b>${esc(projects.length)}</b> saved projects</span><span><b>${esc(strongIdeas)}</b> strong opportunities found</span><span><b>${money(cartTotal)}</b> estimated prototype spend</span></div></section>
    <section id="projects" class="projects-section" aria-labelledby="projects-title">${sectionHeader("Explore", "Projects", "Each card shows its next action. Open one only when you want the full brief.", "projects-title")}<div class="toolbar"><div class="filter-group" role="group" aria-label="Filter projects by status"><button class="filter-button active" type="button" data-status-filter="all">All <span>${esc(projects.length)}</span></button>${projectStatuses.map((status) => `<button class="filter-button" type="button" data-status-filter="${esc(status)}">${esc(statusLabel(status))} <span>${projects.filter((project) => String(project.status || "unknown").toLowerCase() === status).length}</span></button>`).join("")}</div><label class="sort-control" for="project-sort">Sort <select id="project-sort"><option value="priority">Priority first</option><option value="score">Score first</option><option value="title">Title A–Z</option></select></label></div><div class="project-grid-list" data-project-list>${projects.length ? projects.map((project, index) => renderProject(project, cartById, index)).join("") : emptyState("No projects loaded", "Add project JSON files to ./projects and rebuild the dashboard.")}</div><p class="filter-empty" data-filter-empty hidden>No projects match this filter.</p></section>
    <section id="research" class="research-section" aria-labelledby="research-title">${sectionHeader("Look up", "Evidence & costs", "Supporting material for a project decision. It stays closed until you need it.", "research-title")}<div class="research-columns"><div><h3 class="subsection-title">Cost sheets</h3>${carts.length ? `<div class="cart-list">${carts.map((cart) => renderCart(cart, projectById)).join("")}</div>` : emptyState("No carts loaded", "Add BOM JSON files to ./carts and rebuild the dashboard.")}</div><div><h3 class="subsection-title">Research scans</h3>${latest ? renderScan(latest, true) : emptyState("No daily scan yet", "Append a JSON object to ./log.jsonl, then rebuild the dashboard.")}</div></div>${entries.length > 1 ? `<div class="archive-list">${entries.slice(1).map((entry) => renderScan(entry)).join("")}</div>` : ""}</section>
  </main>
