  ...
  return `<article class="project-card" data-project-card data-status="${esc(String(project.status || "unknown").toLowerCase())}" data-priority="${esc(number(project.priority, "99"))}" data-score="${esc(number(scores.total, "0"))}">
    <button class="project-toggle" type="button" aria-expanded="false" aria-controls="${esc(detailsId)}">
      <span class="project-head">
        ...
        <span class="project-status-row"><span class="status-badge status-${esc(statusClass(project.status))}">${esc(statusLabel(project.status))}</span><span class="project-chevron" aria-hidden="true"></span></span>
      </span>
      <span class="project-toggle-score">${renderScoreStrip(scores)}</span>
    </button>
...
function renderFocus(project) {
  ...
  const nextAction = Array.isArray(project.nextActions) && project.nextActions[0]
    ? project.nextActions[0]
    : "Choose the next concrete action.";
  return `<article class="focus-card">
    <div class="focus-label"><span class="signal-dot"></span> Work on this next</div>
    <div class="focus-content"><div><p class="eyebrow">Priority ${esc(number(project.priority))} · ${esc(statusLabel(project.status))}</p><h2>${text(project.title, "Untitled project")}</h2><p class="focus-problem">${text(project.problem)}</p></div>
    <div class="next-move"><span class="label">Next move</span><p>${esc(nextAction)}</p><a class="text-link" href="#projects">Open project details <span aria-hidden="true">↓</span></a></div></div>
  </article>`;
}
...
<body>
  ...
  <header class="hero"><div class="shell">
    <nav class="topbar" aria-label="Dashboard navigation"><a class="brand" href="#top"><span class="brand-mark">D</span><span><b>Build OS</b><small>George's dementia tech lab</small></span></a><div class="top-links"><a href="#now">Now</a><a href="#projects">Projects</a><a href="#research">Research</a></div></nav>
  <div class="hero-grid" id="top"><div><p class="eyebrow amber">Last scan · ${esc(formatDate(latest && latest.date))}</p><h1>Dementia Tech<br><em>Build OS</em></h1><p class="hero-copy">A short, decision-first workspace for turning caregiver problems into useful products.</p><div class="hero-actions"><a class="primary-button" href="#now">See next move <span aria-hidden="true">↓</span></a><a class="outline-button" href="#projects">All projects <span aria-hidden="true">↓</span></a></div></div><aside class="hero-note"><span class="signal-dot"></span><div><b>Research status</b><p>${latest ? `Scan ${latest.accessOk ? "ready" : "needs attention"} · ${esc(strongIdeas)} strong ideas found` : "No daily scan has been loaded yet."}</p></div><hr><p class="muted">${esc(projects.length)} projects · ${esc(carts.length)} cost sheets</p></aside></div>
  </div></header>
  <main class="shell">
    <section id="now" class="now-section" aria-labelledby="now-title">${sectionHeader("Start here", "What to work on now", "One project, one next move. Everything else is available when you need it.", "now-title")}${renderFocus(focusProject)}<div class="quick-stats"><span><b>${esc(projects.length)}</b> active projects</span><span><b>${esc(strongIdeas)}</b> validated opportunities</span><span><b>${money(cartTotal)}</b> estimated prototype spend</span></div></section>
    <section id="projects" class="projects-section" aria-labelledby="projects-title">${sectionHeader("Portfolio", "Projects", "Open only the project you want to evaluate; details stay out of the way until then.", "projects-title")}<div class="toolbar"><div class="filter-group" role="group" aria-label="Filter projects by status"><button class="filter-button active" type="button" data-status-filter="all">All <span>${esc(projects.length)}</span></button>${projectStatuses.map((status) => `<button class="filter-button" type="button" data-status-filter="${esc(status)}">${esc(statusLabel(status))} <span>${projects.filter((project) => String(project.status || "unknown").toLowerCase() === status).length}</span></button>`).join("")}</div><label class="sort-control" for="project-sort">Sort <select id="project-sort"><option value="priority">Priority first</option><option value="score">Score first</option><option value="title">Title A–Z</option></select></label></div><div class="project-grid-list" data-project-list>${projects.length ? projects.map((project, index) => renderProject(project, cartById, index)).join("") : emptyState("No projects loaded", "Add project JSON files to ./projects and rebuild the dashboard.")}</div><p class="filter-empty" data-filter-empty hidden>No projects match this filter.</p></section>
    <section id="research" class="research-section" aria-labelledby="research-title">${sectionHeader("Reference", "Research & costs", "Open a cost sheet or scan only when it helps the decision in front of you.", "research-title")}<div class="research-columns"><div><h3 class="subsection-title">Cost sheets</h3>${carts.length ? `<div class="cart-list">${carts.map((cart) => renderCart(cart, projectById)).join("")}</div>` : emptyState("No carts loaded", "Add BOM JSON files to ./carts and rebuild the dashboard.")}</div><div><h3 class="subsection-title">Research scans</h3>${latest ? renderScan(latest, true) : emptyState("No daily scan yet", "Append a JSON object to ./log.jsonl, then rebuild the dashboard.")}</div></div>${entries.length > 1 ? `<div class="archive-list">${entries.slice(1).map((entry) => renderScan(entry)).join("")}</div>` : ""}</section>
  </main>
